import ClothSVG from "./clothSVG.js";
...
export class SubPageCloth {
  ...
  mainPageHtml(data) {
    ...
    for (let i in clothTag) {
      clothImg[i] = `<div class="clothImges" value="${i}">${
        ClothSVG[clothTag[i]]
      }</div>`;
    ...
    let mainIndex = Math.floor(clothTag.length / 2)
    let clothMain = clothImges[mainIndex];
    ...
    const clothAnimation = () => {
      this.clothtimeout = setTimeout(() => {
        movecloth(5);
        this.clothinterval = setInterval(() => {
          movecloth(5);
        }, 7000);
      }, 3000);
    };
    ...
    function movecloth(time) {
      ...
      clothMain = clothImges[mainIndex];
    }

    let clothTagBtnS = document
      .getElementsByClassName("clothTag")[0]
    ...
    for (let i = 0; i < clothTagBtnS.length; ++i) {
      ...
      clothTagBtn[i].addEventListener("click", () => {
        clearInterval(this.clothinterval);
        clearTimeout(this.clothtimeout);
        clothAnimation();
