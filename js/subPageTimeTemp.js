...
export class SubPageTimeTemp {
  constructor(data) {
    ...
    // 회살표 클릭
    this.chartArrowFunc = this.clickPrevNext.bind(this)

    this.timeTemp.addEventListener("click", this.timeTempFunc);
    this.CloseBtn.addEventListener("click", this.CloseBtnFunc);
    document.addEventListener("click", this.chartArrowFunc);

    let backGroundColorPick = this.body.style.background
      .split("(")[4]
      .split(")")[0];
    if (this.body.style.background.split("(")[5]) {
      backGroundColorPick = this.body.style.background
        .split("(")[5]
        .split(")")[0];
    }
    document.documentElement.style.setProperty(
      "--btn-color",
      `rgba(${backGroundColorPick},1)`
    );
  }

  timeTempChartSizing(timeTempDivCount) {
    this.hourTempDivWidth =
      (this.body.getBoundingClientRect().width -
        this.body.getBoundingClientRect().width * 0.06) /
      (timeTempDivCount + 1);
  ...
  htmlInAPI(data) {
    ...
    for (let i = 0; i < data.hourly.length; ++i) {
      ...
      div.innerHTML = `
        ...
        <div>
          <img src="images/${
            this.AnimationAndDesign.icon(data.hourly[i].weather[0].icon)[0]
          }" alt="icon ${i}">
        </div>
        ${hour}
      `;
      ...
      this.timeicon.style.width = this.AnimationAndDesign.icon(
        data.hourly[i].weather[0].icon
      )[1];

      this.tempPersent =
        10 + ((Math.round(data.hourly[i].temp) - min) / range) * 70;
      this.chartPoint[i].style.bottom = `${this.tempPersent}%`;

      let pointX =
        this.hourTempDivWidth * (i + 1) - this.hourTempDivWidth / 2 + 1;
      ...
      if (i - 1 >= 0) {
        let tP =
          10 + ((Math.round(data.hourly[i - 1].temp) - min) / range) * 70;
    ...
    for (let i = 0; i < data.daily.length; ++i) {
      ...
      this.weekTemp.innerHTML += `
        <article>
          ...
          <div>
            <img src="images/${
              this.AnimationAndDesign.icon(data.daily[i].weather[0].icon)[0]
            }" alt="icon ${i}">
          </div>
        </article>
      `;
      ...
      this.weekIcon.style.width = this.AnimationAndDesign.icon(
  ...
  distroid() {
    this.timeTemp.removeEventListener("click", this.timeTempFunc);
    this.CloseBtn.removeEventListener("click", this.CloseBtnFunc);
    document.removeEventListener("click", this.chartArrowFunc);
  }
}
