...
export class SubPageNowWeather {
  constructor() {
    ...
    this.CloseBtn =
      this.subPageNowWeather.getElementsByClassName("CloseBtn")[0];
  ...
  htmlInAPI(data, curMintemp, curMaxtemp) {
    ...
    this.windHumidity.innerHTML = `
      ...
      <div>
        ...
        <p>${Math.floor(data.daily[0].pop * 100)} <span>%</span></p>
      </div>
      <div>
    ...
    function getUviLevelInfo(i) {
      if (2.99 >= i) {
        return [
          "낮음",
          "rgba(62, 168, 46, 0.4)",
          "외출할 때 자외선 차단제를 얇게 발라주면 피부가 좋아할 거예요.",
        ];
      } else if (i >= 3 && i <= 5.99) {
        return [
          "보통",
          "rgba(255, 245, 0, 0.4)",
          "자외선 차단제를 꼼꼼히 발라주세요. 모자와 선글라스를 쓰셔도 좋아요.",
        ];
      } else if (i >= 6 && i <= 7.99) {
        return [
          "높음",
          "rgba(240, 140, 0, 0.4)",
          "해가 뜨거워요! 자외선 차단제를 챙겨 정기적으로 발라주세요.",
        ];
      } else if (i >= 8 && i <= 10.99) {
        return [
          "매우높음",
          "rgba(228, 51, 15, 0.4)",
          "완전 내리쬐어요 ! 자외선 차단제를 챙겨 정기적으로 바르고 해를 피해야 해요.",
        ];
      } else if (i >= 11) {
        return [
          "위험",
          "rgba(180, 103, 161, 0.4)",
          "밖은 위험해 ! 가능한 실내에 있어야 해요. 자외선 차단제는 필수 !",
        ];
      }
    }
    ...
    this.uvi.innerHTML = `
      <div>
        ...
        <span>${getUviLevelInfo(data.current.uvi)[0]}</span>
      </div>
      <div>
        <p>오늘 최대</br>자외선 지수</p>
        <span>${data.daily[0].uvi}</span>
        <span>${getUviLevelInfo(data.daily[0].uvi)[0]}</span>
      </div>
    ...
    this.tomorrowUvi.innerHTML = `
      <div>
        <p>내일 최대</br>자외선 지수</p>
        <span>${data.daily[1].uvi}</span>
        <span>${getUviLevelInfo(data.daily[1].uvi)[0]}</span>
      </div>
      <h4>${getUviLevelInfo(data.daily[1].uvi)[2]}</h4>
    `;
    let tomorrowUviNum = this.tomorrowUvi
      .getElementsByTagName("div")[0]
      .getElementsByTagName("span")[1];
    ...
    if (visibility == 10000) {
      ...
      visibilityNum = `<span>${
        Math.round((visibility / 1000 + Number.EPSILON) * 10) / 10
      }</span><span> km</span>`;
    ...
    this.sunTime.innerHTML = `
      <p>${suntext}</p>
      <span>${("00" + suntime.getHours()).slice(-2)} : ${(
      "00" + suntime.getMinutes()
    ).slice(-2)}</span>
    `;
  ...
  optionInfoHtml(skyState, weatherState, visibility) {
    ...
    const clearTowShowOption = (uvi, cloud) => {
      this.dobbleBtnFunc = () => {
        ...
        this.dobbleBtn.style.display = "none";
        cloud.getElementsByTagName("p")[1].style.display = "none";
        uvi.getElementsByTagName("h4")[0].style.display = "none";
        uvi.getElementsByTagName("span")[0].style.display = "none";
        if (skyState === "d") {
          uvi.getElementsByTagName("span")[2].style.display = "none";
        }
      };

      this.mainFunc = () => {
        if (this.dobbleBtn.style.display == "none") {
          uvi.style.width = "100%";
          cloud.style.display = "none";
          this.dobbleBtn.style.display = "flex";
          uvi.getElementsByTagName("h4")[0].style.display = "block";
          uvi.getElementsByTagName("span")[0].style.display = "block";
          if (skyState === "d") {
            uvi.getElementsByTagName("span")[2].style.display = "block";
          }
        }
      };
      ...
      this.dobbleBtn.addEventListener("click", this.dobbleBtnFunc);
      uvi.addEventListener("click", this.mainFunc);
      cloud.addEventListener("click", this.cloudFunc);
    };
    ...
    switch (weatherState) {
      ...
      case "04":
        ...
        if (skyState == "d") {
          ...
          clearTowShowOption(this.uvi, this.clouds);
        } else if (skyState == "n") {
          this.mainOption = this.clouds;
          this.clouds.style.display = "flex";
          clearTowShowOption(this.tomorrowUvi, this.clouds);
        }
        break;
      case "09":
      case "10":
